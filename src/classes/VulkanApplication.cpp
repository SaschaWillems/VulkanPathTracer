...
VkResult VulkanApplication::createInstance(bool enableValidation)
{
	...
	if (volkInitResult != VK_SUCCESS) {
		vks::tools::exitFatal("Could not create Vulkan instance : Could not initialize volk", volkInitResult);
	}
	...
	VkResult instanceCreateResult = vkCreateInstance(&instanceCreateInfo, nullptr, &instance);
...
void VulkanApplication::displayLinkOutputCb()
{
	if (prepared)
		nextFrame();
}

void VulkanApplication::mouseDragged(float x, float y)
{
	handleMouseMove(static_cast<uint32_t>(x), static_cast<uint32_t>(y));
}

void VulkanApplication::windowWillResize(float x, float y)
{
	resizing = true;
	if (prepared)
	{
...
void VulkanApplication::windowDidResize()
{
	resizing = false;
}
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
IDirectFBSurface *VulkanApplication::setupWindow()
{
	DFBResult ret;
	int posx = 0, posy = 0;
...
void VulkanApplication::handleEvent(const DFBWindowEvent *event)
{
	switch (event->type)
	{
	case DWET_CLOSE:
